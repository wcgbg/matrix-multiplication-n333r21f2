...
public:
  ...
  using Vec = GFVec<kP, Problem::kNA>;
  ...
  // known lower bound.
  void Set(const Constraints<Problem::kP, Problem::kNA> &c, int rank);
  ...
  int Get(const Constraints<Problem::kP, Problem::kNA> &q,
...
private:
  ...
  using Map =
      boost::unordered_flat_map<Constraints<Problem::kP, Problem::kNA>, Value,
...
template <class Problem>
void OrbitMap<Problem>::Set(const Constraints<Problem::kP, Problem::kNA> &c,
                            int rank) {
  CHECK_GE(rank, 0);
  CHECK_LT(rank, 256);
  // Insert one entry per store-image of `c`. Each image is the subspace
  // store.Apply(store_elem, ·) applied to `c`, re-normalized to the canonical
  ...
  Constraints<Problem::kP, Problem::kNA> image;
  for (int j = 0; j < store_size; ++j) {
    ...
    const int image_rank = GaussJordanRREF<Problem::kP, Problem::kNA>(&image);
    CHECK_EQ(image_rank, static_cast<int>(c.size()));
    const std::size_t shard =
        ConstraintsHash<Problem::kP, Problem::kNA>{}(image) % kNumShards;
...
template <class Problem>
int OrbitMap<Problem>::Get(const Constraints<Problem::kP, Problem::kNA> &q,
                           QueryElem *witness_query,
                           StoreElem *witness_store) const {
  // Probe the map: for each query element, normalize query.Apply(query, q)
