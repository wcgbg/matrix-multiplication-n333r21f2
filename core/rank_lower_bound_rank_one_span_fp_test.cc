...
TEST(RankOneSpanFpTest, DiagonalF3) {
  ...
  t[2][2][2] = GF<3>{1};
  auto fp = [&](int target) {
    return RankOneSpanExcludeFpA<3, 3, 3, 3>(t, target, kBudget);
  };
...
TEST(RankOneSpanFpTest, MatMul122F3) {
  ...
  auto fp = [&](int target) {
    return RankOneSpanExcludeFpA<3, 2, 4, 2>(t, target, kBudget);
  };
...
TEST(RankOneSpanFpTest, MatMul123F3AlongC) {
  ...
  auto along_b = [&](int target) {
    return RankOneSpanExcludeFpA<3, 6, 3, 2>(t1, target, kBudget);
  };
...
void CompareEngines(const Tensor<2, A, B, C> &tensor, int target,
                    const std::string &label, int *compared) {
  ...
  const auto fp =
      RankOneSpanExcludeFpA<2, A, B, C>(tensor, target, kAgreeBudget);
...
void AgreeWithF2Engine(const std::string &path) {
  ...
  for (const pb::ConstrainedTensor &rt : cert.constrained_tensors()) {
    const auto constraints =
        ConstraintsFromBytes<2, static_cast<int>(NA)>(rt.constraints());
    ...
    for (int target = std::max(0, lb - 2); target <= lb; ++target) {
      const std::string label = path + " orbit " + std::to_string(rt.index()) +
...
TEST(RankOneSpanFpTest, NeverContradictsF3Certificate) {
  ...
  const pb::Certificate cert = ReadProtoFromFile<pb::Certificate>(
      "certs/matrix/cert_matrix_q03_n233.pb.txt");
  const auto full = Problem::MakeTensor();
  int decided = 0;
  for (const pb::ConstrainedTensor &rt : cert.constrained_tensors()) {
    const auto constraints = ConstraintsFromBytes<3, 6>(rt.constraints());
    const Tensor<3, 6, 9, 6> t =
        ApplyConstraintsToTensor<3, 6, 9, 6>(constraints, full);
    const int lb = rt.rank_lower_bound();
    if (lb == 0) {
      continue;
    }
