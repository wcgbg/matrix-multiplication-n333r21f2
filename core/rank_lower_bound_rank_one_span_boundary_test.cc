...
void RunTrials(uint64_t seed, int trials, int mode, int *decided,
               int *boundary) {
...
//   f1 (x) o1 + f2 (x) o1',  f1 (x) o2 + f2 (x) o2'      (f1, f2 share the line
//   D)
...
TEST(FamilySearchBoundaryTest, PlantedDoubledLineWitnesses) {
  ...
  for (int trial = 0; trial < 600; ++trial) {
    ...
    if (span.rho != 6 || span.rb != 3 || span.rc != 5)
      continue;
    ...
    const RankOneSpanResult v1 =
        EnumerateAllSubspaces<2, 6, 3, 5>(span, target);
    uint64_t ops = 0;
    const RankOneSpanResult v2 =
    ...
    if (v2 == RankOneSpanResult::kOverBudget)
      continue;
    ++checked;
    if (v1 == RankOneSpanResult::kWitnessFound)
      ++witnesses;
...
TEST(FamilySearchBoundaryTest, AgreesWithV1NearCoverageBoundary) {
  ...
  GTEST_LOG_(INFO) << "decided " << decided << ", boundary-regime " << boundary;
}
...
void OrbitStructure(const char *name, const std::vector<uint16_t> &words) {
  ...
  const auto t0 =
      ApplyConstraintsToTensor<2, 9, 9, 9>(constraints, Problem::MakeTensor());
  ...
  for (const auto &row : span.v.rows)
    is_top[row.first] = true;
  std::vector<int> free_pos;
  for (int i = 0; i < rb * rc; ++i)
    if (!is_top[i])
  ...
  for (int f = 1; f <= F; ++f) {
    for (int j = 0; j < odim; ++j)
      image[f].Insert(key_of(make_row(f, uint64_t{1} << j)));
  ...
  LOG(INFO) << name << ": rb=" << rb << " rc=" << rc << " rho=" << rho
            << " dq=" << dq << " F=" << F << " d0=" << d0 << " need4=" << need4
            << " need5=" << need5 << " occupied=" << occupied
            << " multi2=" << multi2 << " lines=" << total_lines
            << " doubled=" << doubled << " tripled=" << tripled
            << " c10_roots=" << c10_roots << " delta_cands=" << delta_cands
            << " subs3=" << subs3 << " c9_roots=" << c9_roots;
}
